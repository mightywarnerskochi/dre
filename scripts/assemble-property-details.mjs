/**
 * Builds PropertyDetailsContent.vue: banner + main + similar (v-for) + trailing close from php (lines 589-611).
 */
import fs from 'fs';
import { fileURLToPath } from 'url';
import pathMod from 'path';
import { transformMarkup } from './php-markup.mjs';

const __dirname = pathMod.dirname(fileURLToPath(import.meta.url));
const root = pathMod.resolve(__dirname, '..');
const phpPath = pathMod.join(root, 'html', 'properties-details.php');
const lines = fs.readFileSync(phpPath, 'utf8').replace(/\r\n/g, '\n').split('\n');

const bannerAndMain = transformMarkup(lines.slice(8, 495).join('\n'));
const openingSimilar = lines.slice(496, 501).join('\n');
const tailClose = transformMarkup(lines.slice(588, 611).join('\n'));
...
const tpl = `${bannerAndMain}\n${openingSimilar}${SLIDES}${tailClose}`;

const scriptSetup = `import { asset } from '@/utils/asset';
...
const outfile = pathMod.join(root, 'resources/js/components/PropertyDetailsContent.vue');
fs.writeFileSync(outfile, `<template>\n${tpl}\n</template>\n\n<script setup>\n${scriptSetup}\n</script>\n`);

console.log('Wrote PropertyDetailsContent.vue');
