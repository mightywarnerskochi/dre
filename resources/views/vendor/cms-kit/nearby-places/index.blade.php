...
@section('breadcrumbs')
    <li class="breadcrumb-item active">Nearby Places</li>
@endsection
...
<div class="card">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
        ...
        <div class="d-flex gap-2">
            @if(auth('cms')->user()->can('nearby-places.delete'))
            ...
            @endif
            @if(auth('cms')->user()->can('nearby-places.create'))
                <a href="{{ route('cms.nearby-places.create') }}" class="btn btn-primary btn-sm">Add Place</a>
            @endif
        </div>
    </div>
    <div class="card-body p-4">
        <div class="table-responsive">
            <table class="table premium-table mb-0 w-100" id="nearbyPlacesTable">
                <thead>
                    <tr>
...
<script>
$(function () {
    const table = $('#nearbyPlacesTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('cms.nearby-places.index') }}",
        columns: [
    ...
    window.bulkAction = function(action) {
        if (action === 'delete' && !confirm('Delete selected nearby places?')) return;
        $.post("{{ route('cms.nearby-places.bulk-action') }}", {
    ...
    $(document).on('click', '.delete-item', function() {
        if (!confirm('Delete this nearby place?')) return;
        $.ajax({
            url: "{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/nearby-places/" + $(this).data('id'),
            type: 'DELETE',
    ...
    $(document).on('change', '.toggle-status', function() {
        $.post("{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/nearby-places/" + $(this).data('id') + "/toggle-status", {_token: "{{ csrf_token() }}"})
