...
<div class="property-details-page-wrap property-details-page-inner">
    <p v-if="loading" class="container-ctn py-5 text-center text-muted">{{ t('propertyDetail.loading') }}</p>
    <p v-else-if="!property" class="container-ctn py-5 text-center">{{ t('propertyDetail.notFound') }}</p>
    <template v-else>
<section class="banner banner--page banner--page--listing position-relative" :aria-label="t('listing.pageHeaderAria')">
        <div class="banner--page__bg">
            <picture>
                <img :src="heroBannerSrc" alt="Property banner" width="1920" height="403" loading="eager">
            </picture>
        </div>
        <div class="banner--page__content">
            <div class="container-ctn">
                ...
                <ol class="breadcrumb-minimal" aria-label="Breadcrumb">
                    ...
                    <li><RouterLink :to="{ name: 'our-property' }">{{ t('home.rentals.card.propertyType') }}</RouterLink></li>
                    <li class="breadcrumb-minimal__sep" aria-hidden="true">/</li>
    ...
    <div class="property-detail-main">
        ...
        <div class="property-detail-gallery position-relative">
  ...
  <path d="M15 11.9998L16.644 10.3558C16.7564 10.2433 16.8898 10.1541 17.0367 10.0932C17.1836 10.0323 17.341 10.001 17.5 10.001C17.659 10.001 17.8164 10.0323 17.9633 10.0932C18.1102 10.1541 18.2436 10.2433 18.356 10.3558L21 12.9998" stroke="#2F3F58" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        ...
                        <span class="property-detail-gallery__all-label">{{ t('propertyDetail.allPhotos') }}</span>
                    </button>
                </div>
                                ...
                                <div class="property-detail-agent__header">
                                    <div class="property-detail-agent__profile">
                                        <div class="property-detail-agent__avatar">
                                            <img :src="agentAvatar" alt="Property agent" width="64" height="64" loading="lazy" @error="dreOnAgentImgError">
                                        </div>
                                ...
                                <div class="property-detail-agent__body">
                                    <div v-if="agentExperience || agentLanguages" class="property-detail-agent__meta">
                                        <p v-if="agentExperience">
                                            <strong>{{ t('propertyDetail.experience') }} :</strong>
                                            <span>{{ agentExperience }}</span>
                                        </p>
                                        <p v-if="agentLanguages">
                                            <strong>{{ t('propertyDetail.languages') }} :</strong>
        ...
        >
            <div class="property-detail-body__main _left_col">
                <section v-if="descriptionHtml" class="property-detail-section" aria-labelledby="pd-desc-heading">
                    <h3 id="pd-desc-heading" class="property-detail-section__title">{{ t('propertyDetail.description') }}</h3>
                ...
                <section v-if="accessPlaces.length" class="property-detail-section" aria-labelledby="pd-access-heading">
                    <h3 id="pd-access-heading" class="property-detail-section__title">{{ t('propertyDetail.easyAccess') }}</h3>
                ...
                <section v-if="specCells.length" class="property-detail-section" aria-labelledby="pd-specs-heading">
                    <h3 id="pd-specs-heading" class="property-detail-section__title">{{ t('propertyDetail.detailsTitle') }}</h3>
                    <div class="property-detail-specs property-detail-specs--cards property-detail-specs--detail-cards">
                        <div v-for="cell in specCells" :key="cell.id" class="property-detail-specs__cell">
                            <span class="property-detail-specs__icon" aria-hidden="true"><img :src="cell.icon" alt="Property spec icon" width="24" height="24" loading="lazy"></span>
                ...
                <section v-if="featureList.length || amenityCards.length || propertyAttributeCards.length" class="property-detail-section" aria-labelledby="pd-features-heading">
                    <h3 id="pd-features-heading" class="property-detail-section__title">{{ t('propertyDetail.keyFeatures') }}</h3>
                    ...
                    <template v-if="amenityCards.length">
                        <h4 class="property-detail-section__subtitle">{{ t('propertyDetail.amenities') }}</h4>
                    ...
                    <template v-if="propertyAttributeCards.length">
                        <h4 class="property-detail-section__subtitle">{{ t('propertyDetail.attributes') }}</h4>
                        ...
                        <div class="property-detail-calc js-payment-calc">
                            ...
                            <div class="property-detail-calc__layout">
                                <div class="property-detail-calc__donut-wrap">
                                    <div class="property-detail-calc__donut js-calc-donut" role="img" :aria-label="t('propertyDetail.paymentChartAria')">
        ...
        <section v-if="similar.length" class="property-detail-similar commonPadding-120 pb-0" aria-labelledby="similar-listings-heading">
            <div class="container-ctn">
                <h2 id="similar-listings-heading" class="property-detail-similar__head">{{ t('propertyDetail.similarListing') }}</h2>
                            ...
                            <article class="property-card property-card--listing" :data-property-url="similarCardDataPropertyUrl(p)">
                                ...
                                <div class="property-card__inner">
                                    <div class="property-card__media">
                                        <span v-if="p.is_featured" class="property-card__badge">{{ t('listing.featured') }}</span>
                                        <picture>
                                            <img
                                                :src="similarImage(p)"
                                                alt="Property image"
                                    ...
                                    <div class="property-card__body">
                                        ...
                                        <div class="property-details">
                                            <div class="property-details__item">
                                                ...
                                                <span>{{ t('listing.bedroomsUnit', Number(p.bedrooms) || 0, { count: Number(p.bedrooms) || 0 }) }}</span>
                                            </div>
                                            <div class="property-details__item">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                                ...
                                                <span>{{ t('listing.bathroomsUnit', Number(p.bathrooms) || 0, { count: Number(p.bathrooms) || 0 }) }}</span>
                                            </div>
                                            <div class="property-details__item">
...
function galleryAlt(i) {
    return `Property photo ${i + 1}`;
}

const propertyTypeLabel = computed(() => {
    return property.value?.propertyTypeLabel || property.value?.property_type || t('home.rentals.card.propertyType');
});

const listingOutlineLabel = computed(() => {
    return property.value?.listingTypeLabel || property.value?.listing_type || '';
});
...
const agentRole = computed(() => {
    return property.value?.agent?.designation || '';
});
const agentExperience = computed(() => {
    return property.value?.agent?.experience || '';
});
const agentLanguages = computed(() => {
    return property.value?.agent?.languages || '';
});
...
const amenityCards = computed(() => {
    ...
    return rows
        .map((item, index) => {
            ...
            const label = (locale.value === 'ar' ? (item?.label_ar || item?.title_ar || item?.name_ar || item?.value_ar) : null)
                || item?.label || item?.title || item?.name || item?.value || '';

            return {
                id: item?.label || item?.title || `amenity-${index}`,
                label: label || (locale.value === 'ar' ? 'مرفق' : 'Amenity'),
...
const featureList = computed(() => {
    ...
    return rows
        .map((item) => {
            ...
            const val = (locale.value === 'ar' ? (item?.text_ar ?? item?.label_ar ?? item?.title_ar ?? item?.value_ar ?? item?.description_ar) : null)
                || item?.text
                || item?.label
                || item?.title
                || item?.value
...
const propertyAttributeCards = computed(() => {
    ...
    return rows
        .map((item, index) => {
            const raw = (locale.value === 'ar' ? (item?.label_ar ?? item?.name_ar ?? item?.title_ar ?? item?.value_ar) : null)
                || (typeof item === 'string' ? item : (item?.label ?? item?.name ?? item?.title ?? item?.value ?? ''));
...
const accessPlaces = computed(() => {
    ...
    return rows
        .map((item, index) => {
            ...
            const name = (locale.value === 'ar' ? item?.name_ar : null) || String(item?.name || '').trim();

            return {
                id: `${name || 'place'}-${index}`,
                name: name || (locale.value === 'ar' ? 'موقع قريب' : ''),
...
function listingLabel(listingType) {
    ...
    if (type === 'rent') return t('listing.rentals');
    if (type === 'sale') return t('listing.forSale');
...
watch(
    ...
    ([value, activeLocale]) => {
        ...
        applySeoPayload(value.seo || {}, activeLocale, {
            title: value.title || 'Property',
            description: plainText(value.description || value.full_address || value.location || value.title || 'Property'),
            canonicalUrl: window.location.href,
            ogImage: galleryImages.value[0] || DRE_PROPERTY_PLACEHOLDER_IMAGE,
        });
